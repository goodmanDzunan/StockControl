...
namespace StockControl_2
{
    public partial class CfrmStockControl : Form
    {
        ...
        private void btnReceive_Click(object sender, EventArgs e)
        {
            ...
            string itemCode = Interaction.InputBox("Enter item code of the item to receive stock:", "STOCK CONTROL", "", 200, 200);
            ...
            {
                //quantity to add
                string quantityN = Interaction.InputBox("Enter quantity to add:","STOCK CONTROL","",200,200);

                //validate input
                if(int.TryParse(quantityN, out int quantityToAdd) && quantityToAdd > 0)
                {
                    //increase quantity
                    item.Quantity += quantityToAdd;
                    
                    MessageBox.Show("Item received successfully.","STOCK CONTROL",MessageBoxButtons.OK, MessageBoxIcon.Information);
                        
                    
                }
                else
                {
                    MessageBox.Show("Invalid quantity entered. Please add a positive number.", "STOCK CONTROL", MessageBoxButtons.OK, MessageBoxIcon.Warning);

                }
            }
            else
            {
                MessageBox.Show("Item not found.","STOCK CONTROL",MessageBoxButtons.OK,MessageBoxIcon.Error);
            }
        }

        private void lblCloseStock_MouseEnter(object sender, EventArgs e)
        {
            lblCloseStock.ForeColor = Color.DarkRed;
            lblCloseStock.BackColor = Color.CadetBlue;
        }

        private void lblCloseStock_MouseLeave(object sender, EventArgs e)
        {
            lblCloseStock.ForeColor = Color.Red;
            lblCloseStock.BackColor = Color.Transparent;
        }
    }
}
