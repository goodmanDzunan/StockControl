...
namespace StockControl_2
{
    public class CItem
    {
        ...
        private static int CompareByCode(CItem item1, CItem item2)
        {
            return string.Compare(item1.ItemCode, item2.ItemCode);
        }
        private static int CompareByName(CItem item1, CItem item2)
        {
            return string.Compare(item1.ItemName, item2.ItemName);

        }
        private int CompareByPrice(CItem item1, CItem item2)
        {
            if (item1.retailPrice > item2.retailPrice)
                return 1;
            else
            {
                if (item1.retailPrice < item2.retailPrice)
